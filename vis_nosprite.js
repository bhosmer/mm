...
export class MatMul extends THREE.Group {
    ...
    constructor(params, getText) {
        ...
        if (this.alg == 'dotprod') {
            ...
            this.I = this.H - 1
            this.K = this.W - 1
        } else if (this.alg == 'itemwise') {
            const itemwise_init = (y, x, h, w) => this.dotprod_val(0, 0, x)
            this.itemwise = Mat.fromInit(1, 1, itemwise_init, params)
            this.add(this.itemwise.points)
            this.add(this.itemwise.group)
    ...
    result_val(y, x) {
        let p = 0.0
        const n = this.alg == 'itemwise' ? this.J : this.left.w
        for (let z = 0; z < n; z++) {
